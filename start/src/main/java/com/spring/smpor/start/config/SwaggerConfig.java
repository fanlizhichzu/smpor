...
public class SwaggerConfig {
    ...
    @Bean
    public Docket gitTutorialRestApi() {
        //swagger设置，基本信息，要解析的接口及路径等
        return new Docket(DocumentationType.OAS_30)
                .groupName("gitTutorial")
                .apiInfo(apiInfo())
                .select()
                //设置通过什么方式定位需要自动生成文档的接口，这里定位方法上的@ApiOperation注解
                .apis(RequestHandlerSelectors.basePackage("com.spring.smpor.gistutorial.controller"))
                //接口URI路径设置，any是全路径，也可以通过PathSelectors.regex()正则匹配
                .paths(PathSelectors.any())
                .build();
    }
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder()
                ...
                .contact(new Contact("fanlz", "www.baidu.com", "lizh.fan@outlook.com"))
                .version("1.0")
                .build();
    }
}
