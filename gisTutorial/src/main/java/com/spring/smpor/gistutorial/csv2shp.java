...
public class csv2shp {
    ...
    public static void main(String[] args) throws Exception {
        // TEST
        Point pointByType = createPointByType("Point (1 1)");
    ...
    private static SimpleFeatureType createFeatureType(){
        ...
        typeBuilder.setCRS(DefaultGeographicCRS.WGS84); //  <- Coordinate reference system

        // add attributes in order
        typeBuilder.add("the_geom",Point.class);
        typeBuilder.length(15).add("Name",String.class); // <- 15 chars width  for name field
        typeBuilder.add("number",Integer.class);

        // build the type
        final SimpleFeatureType LOCATION = typeBuilder.buildFeatureType();
     ...
     */
    private static <T> Point createPointByType(T t){
        GeometryFactory geometryFactory = JTSFactoryFinder.getGeometryFactory(null);
        try {
            if (t instanceof String){
                WKTReader wktReader = new WKTReader(geometryFactory);
                return (Point) wktReader.read((String) t);
            }else if (t instanceof Coordinate){
                return geometryFactory.createPoint((Coordinate) t);
            }
        }catch (Exception e){
